...
const SurveyScreen = () => {
    ...
    const fetchSurveyQuestions = async () => {
      try {
        const response = await axios.get(
          "https://survey-app-server-side.vercel.app/getQuestions"
        );
    ...
    const startSession = async () => {
      try {
        ...
        const response = await axios.post(
          "https://survey-app-server-side.vercel.app/startSession",
          {
            sessionId: sessionId,
          }
        );
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              ...
              await axios.post(
                "https://survey-app-server-side.vercel.app/surveyAnswers",
                {
  ...
  const addQuestion = async () => {
    if (newQuestion) {
      try {
        const response = await axios.post(
          "https://survey-app-server-side.vercel.app/addQuestion",
          {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-blue-300">
      {loading ? (
        ...
        <>
          <h1 className="text-4xl font-bold mb-28 text-white">
            Customer Survey
          </h1>
